...
class MessageLayer(MessagePassing):
    ...
    def update(self, agg_message, s, v):
        delta_s, delta_v = agg_message
        U = self.linear_U(delta_v) # [3, natoms, num_embeddings]
        V = self.linear_V(delta_v) # [3, natoms, num_embeddings]

        stack = torch.cat([delta_s, torch.norm(V, dim=0)], dim=1) # [natoms, 2*num_embeddings]
